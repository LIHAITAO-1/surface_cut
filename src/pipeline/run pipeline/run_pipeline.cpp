...
//    triangle_soup_surface_mesh_normal_fix(meshCurve);
    ...
    {
        auto triCube = (Face *)meshCube.face_pool[i];
        tri1 = Triangle3d(triCube->p1->position, triCube->p2->position, triCube->p3->position);
        for (int j = 0; j < meshCurve.face_pool.size(); j++)
        {
            auto triCurve = (Face *)meshCurve.face_pool[j];
            tri2 = Triangle3d(triCurve->p1->position, triCurve->p2->position, triCurve->p3->position);
            if (ComputeLineWithTwoTriangle(tri1, tri2, line))
            {
//              allocate_from_pool
                orig = Vertex::allocate_from_pool(&meshResult.vertex_pool, line.p1);
                end = Vertex::allocate_from_pool(&meshResult.vertex_pool, line.p2);
                Edge::allocate_from_pool(&meshResult.edge_pool, orig, end);
            }
        }
    }
...
//        Face *f = Face::allocate_from_pool(&mesh.face_pool, v, v, v);
    ...
    meshResult.save("D:/xmy/model/", "output");
